...
def mock_adapters():
    ...
    with patch('adapters.langchain_adapter.ChatOpenAI') as mock_llm_class, \
...
class TestEndToEndWorkflow:
    ...
    def test_complete_workflow_ingest_and_query(
        self, 
        test_config, 
        swiggy_pdf_path,
        mock_adapters
    ...
    def test_persistence_workflow_save_and_load(
        self,
        test_config,
        swiggy_pdf_path,
        mock_adapters
    ...
    def test_multiple_queries_on_same_document(
        self,
        test_config,
        swiggy_pdf_path,
        mock_adapters
    ...
    def test_query_with_no_relevant_context(
        ...
        assert result.answer.confidence == "not_found" or \
               "could not find" in result.answer.text.lower() or \
               "not found" in result.answer.text.lower()
...
class TestErrorRecoveryScenarios:
    ...
    def test_query_before_ingestion(self, test_config, mock_adapters):
        ...
        assert result is not None
        assert result.answer is not None
        assert "not initialized" in result.answer.text.lower() or \
               "ingest" in result.answer.text.lower()
    ...
    def test_query_with_empty_question(self, test_config, mock_adapters):
        ...
        assert result is not None
        assert result.answer is not None
        assert "error" in result.answer.text.lower() or \
               "empty" in result.answer.text.lower() or \
               "cannot" in result.answer.text.lower()
    ...
    def test_recovery_after_failed_ingestion(
        self,
        test_config,
        temp_dir,
        mock_adapters
