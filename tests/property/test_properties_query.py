...
class TestQueryHandlerProperties:
    ...
    @given(question=st.text(min_size=1, max_size=1000))
    ...
    def test_property_11_query_validation(self, embedding_model_cached, question):
        ...
        **Validates: Requirements 5.1, 5.2**
        ...
        assert isinstance(result, Embedding), "process_question should return Embedding instance"

        # 2. Embedding should have a vector attribute
        assert hasattr(result, "vector"), "Embedding should have vector attribute"
        assert result.vector is not None, "Embedding vector should not be None"

        # 3. Vector should be a numpy array (numerical vector)
        assert isinstance(result.vector, np.ndarray), "Embedding vector should be a numpy array"

        # 4. Vector should have non-zero values
        # At least some values should be non-zero
        assert np.any(result.vector != 0), "Embedding vector should contain non-zero values"

        # 5. Vector should have valid numerical values (not NaN or Inf)
        assert not np.any(np.isnan(result.vector)), "Embedding vector should not contain NaN values"
        assert not np.any(
            np.isinf(result.vector)
        ...
        assert (
            len(result.vector) == expected_dim
        ...
        assert hasattr(result, "source_text"), "Embedding should have source_text attribute"
        assert result.source_text == question, "Embedding source_text should match input question"

        # 9. Vector values should be in reasonable range
        # For normalized embeddings, values typically in [-1, 1]
        assert np.all(result.vector >= -1.5), "Embedding values should be >= -1.5"
        assert np.all(result.vector <= 1.5), "Embedding values should be <= 1.5"

        # 10. Vector should have sufficient variance (not all same value)
        # This ensures the embedding is meaningful
        assert (
            np.std(result.vector) > 0.01
    ...
    def test_property_11_query_validation_consistency(self, embedding_model_cached, questions):
        ...
        **Validates: Requirements 5.1, 5.2**
        ...
        for i, embedding in enumerate(embeddings):
            ...
            assert embedding.vector is not None, f"Embedding vector at index {i} should not be None"
            assert isinstance(
        ...
        assert all(
            dim == expected_dim for dim in dimensions
...
class TestContextRetrieverProperties:
    ...
    def test_property_12_context_retrieval_completeness(
        self, embedding_model_cached, num_chunks, query_text
    ):
        """
        ...
        **Validates: Requirements 6.1, 6.3**
        ...
        assert len(retrieved_chunks) >= 0, "retrieve_context should return a list (possibly empty)"
        ...
        if len(retrieved_chunks) > 0:
            ...
            for i, rc in enumerate(retrieved_chunks):
                assert hasattr(rc.chunk, "text"), f"Chunk at index {i} should have 'text' attribute"
                assert rc.chunk.text is not None, f"Chunk at index {i} should have non-None text"
                assert isinstance(rc.chunk.text, str), f"Chunk at index {i} text should be a string"
                assert len(rc.chunk.text) > 0, f"Chunk at index {i} should have non-empty text"
            ...
            assert (
                len(retrieved_chunks) <= context_retriever.default_top_k
            ...
            for i, rc in enumerate(retrieved_chunks):
                assert (
                    rc.chunk.metadata.chunk_index in original_chunk_indices
...
class TestAnswerGeneratorProperties:
    ...
    def test_property_13_answer_generation_with_context(self, question, num_chunks):
        """
        ...
        **Validates: Requirements 7.1, 7.4**
        ...
        for i in range(num_chunks):
            ...
            retrieved_chunk = RetrievedChunk(
                chunk=chunk, similarity_score=0.9 - (i * 0.05)  # Decreasing similarity
            )
        ...
        assert isinstance(result, Answer), "generate_answer should return an Answer instance"

        # 2. Answer should have a text attribute
        assert hasattr(result, "text"), "Answer should have 'text' attribute"
        assert result.text is not None, "Answer text should not be None"
        assert isinstance(result.text, str), "Answer text should be a string"
        ...
        assert result.supporting_chunks is not None, "Answer supporting_chunks should not be None"
        assert isinstance(
            result.supporting_chunks, list
        ), "Answer supporting_chunks should be a list"

        # 4. Supporting chunks should include the provided context (Requirement 7.4)
        assert (
            len(result.supporting_chunks) > 0
        ), "Answer should include supporting chunks when context is provided"
        assert len(result.supporting_chunks) == len(
            context_chunks
        ...
        for i, (result_chunk, context_chunk) in enumerate(
            zip(result.supporting_chunks, context_chunks)
        ):
            assert (
                result_chunk.chunk.text == context_chunk.chunk.text
            ), f"Supporting chunk {i} text should match context chunk text"
            assert (
                result_chunk.chunk.metadata.chunk_index == context_chunk.chunk.metadata.chunk_index
            ), f"Supporting chunk {i} metadata should match context chunk metadata"
            assert (
                result_chunk.similarity_score == context_chunk.similarity_score
            ), f"Supporting chunk {i} similarity score should match context chunk score"

        # 7. Answer should have confidence attribute
        assert hasattr(result, "confidence"), "Answer should have 'confidence' attribute"
        assert result.confidence is not None, "Answer confidence should not be None"
        assert isinstance(result.confidence, str), "Answer confidence should be a string"
        ...
        assert (
            call_args[1]["question"] == question or call_args[0][0] == question
        ), "LLM should be called with the provided question"

        # Verify context was passed (should contain chunk texts)
        context_arg = call_args[1].get(
            "context", call_args[0][1] if len(call_args[0]) > 1 else None
        )
        assert context_arg is not None, "LLM should be called with context"
        ...
        assert result.text == mock_answer_text, "Answer text should match the LLM-generated text"

    @given(question=st.text(min_size=5, max_size=200).filter(lambda t: len(t.strip()) > 0))
    @settings(max_examples=100, deadline=15000)
    def test_property_13_answer_generation_empty_context(self, question):
        """
        ...
        **Validates: Requirements 7.1, 7.3, 7.4**
        ...
        assert (
            "could not find" in result.text.lower() or "not found" in result.text.lower()
    ...
    @settings(max_examples=100, deadline=20000)
    def test_property_13_answer_generation_not_found_response(self, question, num_chunks):
        """
        ...
        **Validates: Requirements 7.1, 7.3, 7.4**
        ...
        for i in range(num_chunks):
            ...
            retrieved_chunk = RetrievedChunk(chunk=chunk, similarity_score=0.5 - (i * 0.05))
            context_chunks.append(retrieved_chunk)
        ...
        assert (
            "could not find" in result.text.lower()
    ...
    @settings(max_examples=100, deadline=20000)
    def test_property_14_answer_context_reference(self, question, num_chunks):
        """
        ...
        **Validates: Requirements 7.6**
        ...
        mock_answer_text = (
            f"Based on the context, {distinctive_phrases[0]} is relevant to your question."
        )
        mock_llm.generate_answer.return_value = mock_answer_text
        ...
        references_context = answer_generator.validate_answer_from_context(
            result.text, context_chunks
        )
        assert (
            references_context is True
        ...
        assert len(result.supporting_chunks) > 0, "Answer should include supporting chunks"
        assert len(result.supporting_chunks) == len(
            context_chunks
    ...
    @settings(max_examples=100, deadline=20000)
    def test_property_14_answer_context_reference_no_reference(self, question, num_chunks):
        """
        ...
        **Validates: Requirements 7.6**
        ...
        for i in range(num_chunks):
            ...
            retrieved_chunk = RetrievedChunk(
                chunk=chunk, similarity_score=0.5 - (i * 0.05)  # Lower similarity scores
            )
            context_chunks.append(retrieved_chunk)
        ...
        mock_answer_text = (
            "The answer involves completely unrelated information about weather patterns."
        )
        mock_llm.generate_answer.return_value = mock_answer_text
