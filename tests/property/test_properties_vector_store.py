...
class TestVectorStoreProperties:
    ...
    def test_property_8_vector_store_retrieval_round_trip(
        ...
        **Validates: Requirements 4.1, 4.2**
        ...
        assert vector_store.get_index_size() == num_embeddings, \
            f"Vector store should contain {num_embeddings} embeddings"
        
        # Round-trip test: Search with each stored embedding
        for i, (original_embedding, original_chunk) in enumerate(
            zip(embeddings_list, chunks_list)
        ):
            ...
            assert len(results) > 0, \
                f"Search with stored embedding {i} should return results"
            ...
            for j, result in enumerate(results):
                assert isinstance(result, tuple), \
                    f"Result {j} should be a tuple"
                assert len(result) == 2, \
                    f"Result {j} should be a 2-tuple (Chunk, similarity_score)"
                
                chunk, similarity_score = result
                assert isinstance(chunk, Chunk), \
                    f"Result {j} first element should be a Chunk"
            ...
            retrieved_chunk_indices = [
                chunk.metadata.chunk_index for chunk in retrieved_chunks
            ]
            ...
            assert top_chunk.text == original_chunk.text, \
            ...
            assert top_chunk.metadata.source_document == original_chunk.metadata.source_document, \
                "Retrieved chunk source_document should match original"
            assert top_chunk.metadata.start_position == original_chunk.metadata.start_position, \
                "Retrieved chunk start_position should match original"
            assert top_chunk.metadata.end_position == original_chunk.metadata.end_position, \
    ...
    @settings(max_examples=100, deadline=10000)
    def test_property_9_vector_store_persistence_round_trip(
        self, 
        num_embeddings, 
        embedding_dim
    ):
        ...
        **Validates: Requirements 4.3, 4.4**
        ...
        assert vector_store_original.get_index_size() == num_embeddings, \
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert os.path.exists(f"{index_path}.faiss"), \
                "FAISS index file should be created"
            assert os.path.exists(f"{index_path}.chunks.pkl"), \
            ...
            for i, (original_chunk, loaded_chunk) in enumerate(
                ...
                assert loaded_chunk.text == original_chunk.text, \
                    f"Chunk {i} text should be preserved after save/load"
                assert loaded_chunk.metadata.chunk_index == original_chunk.metadata.chunk_index, \
                    f"Chunk {i} index should be preserved"
                assert loaded_chunk.metadata.source_document == original_chunk.metadata.source_document, \
                    f"Chunk {i} source_document should be preserved"
                assert loaded_chunk.metadata.start_position == original_chunk.metadata.start_position, \
                    f"Chunk {i} start_position should be preserved"
                assert loaded_chunk.metadata.end_position == original_chunk.metadata.end_position, \
            ...
            for j, (orig_result, loaded_result) in enumerate(
                zip(original_results, loaded_results)
            ):
                orig_chunk, orig_distance = orig_result
                loaded_chunk, loaded_distance = loaded_result
                
                assert orig_chunk.metadata.chunk_index == loaded_chunk.metadata.chunk_index, \
    ...
    @settings(max_examples=100, deadline=10000)
    def test_property_10_top_k_retrieval_correctness(
        self, 
        num_embeddings, 
        embedding_dim, 
        k
        ...
        **Validates: Requirements 4.5, 6.2, 6.4**
        ...
        for i, result in enumerate(results):
            assert isinstance(result, tuple), \
                f"Result {i} should be a tuple"
            assert len(result) == 2, \
                f"Result {i} should be a 2-tuple (Chunk, similarity_score)"
            
            chunk, similarity_score = result
            assert isinstance(chunk, Chunk), \
                f"Result {i} first element should be a Chunk"
            assert isinstance(similarity_score, float), \
        ...
        assert len(chunk_indices) == len(set(chunk_indices)), \
            f"Results should not contain duplicate chunks. " \
            f"Found indices: {chunk_indices}"
        ...
        for chunk, _ in results:
            assert chunk.metadata.chunk_index in stored_indices, \
