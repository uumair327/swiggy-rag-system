...
class TestCLIAdapterProperties:
    ...
    def test_property_15_cli_display_completeness(
        ...
        **Validates: Requirements 8.3, 8.4**
        ...
        for i in range(num_chunks):
            chunk_text = f"Supporting chunk {i}: This contains relevant information about the topic. " * 2
            chunk = Chunk(
                text=chunk_text,
        ...
        assert expected_confidence_display in output, \
            f"CLI display should show confidence level: {expected_confidence_display}"
        
        # 4. Output should contain processing time
        assert "Processing Time:" in output or "processing time" in output.lower(), \
            "CLI display should show processing time"
        assert f"{processing_time:.2f}" in output, \
        ...
        assert "SUPPORTING CONTEXT" in output or "supporting context" in output.lower(), \
            "CLI display should have a supporting context section"
        
        # 6. Output should indicate the number of chunks
        assert f"{num_chunks} chunk" in output.lower(), \
    ...
    @settings(max_examples=100, deadline=10000)
    def test_property_15_cli_display_with_scores(
        self,
        answer_text,
        num_chunks,
        processing_time
        ...
        **Validates: Requirements 8.3, 8.4**
        ...
        assert "Similarity Score:" in output or "similarity score" in output.lower(), \
    ...
    @settings(max_examples=100, deadline=10000)
    def test_property_15_cli_display_not_found(
        self,
        prefix_text,
        processing_time
    ):
        ...
        **Validates: Requirements 8.3, 8.4**
        ...
        assert answer_text in output, \
            "CLI display should include the 'not found' answer text"
        ...
        assert "ANSWER" in output, \
            "CLI display should have an ANSWER section"
        ...
        if "SUPPORTING CONTEXT" in output:
            ...
            assert "0 chunk" in output.lower() or not any(f"[Chunk {i}]" in output for i in range(1, 10)), \
    ...
    @settings(max_examples=100, deadline=10000)
    def test_property_15_cli_display_consistency(
        self,
        answer_text,
        num_chunks
    ):
        ...
        **Validates: Requirements 8.3, 8.4**
        ...
        assert answer_index >= 0, \
            "CLI display should have ANSWER section"
        
        if context_index >= 0:  # Only check order if both sections exist
            assert answer_index < context_index, \
        ...
        assert answer_text_index >= 0, \
            "Answer text should appear in output"
        assert confidence_index >= 0, \
            "Confidence should appear in output"
        assert answer_text_index < confidence_index, \
        ...
        assert processing_time_index >= 0, \
            "Processing time should appear in output"
        assert confidence_index < processing_time_index, \
