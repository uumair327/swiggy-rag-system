...
class TestDocumentProcessorProperties:
    ...
    def test_property_1_pdf_text_extraction_completeness(self, pdf_choice):
        ...
        **Validates: Requirements 1.1, 1.2, 1.5**
        ...
        assert result.text is not None, \
            "Extracted text should not be None"
        ...
        assert result.extraction_timestamp is not None, \
            "Extraction timestamp should be present"
        assert len(result.extraction_timestamp) > 0, \
            "Extraction timestamp should not be empty"
        ...
        assert '\n' in result.text, \
            "Extracted text should preserve structure with newlines"
    ...
    def test_property_1_pdf_extraction_consistency(self, pdf_choice, load_twice):
        ...
        **Validates: Requirements 1.1, 1.2, 1.5**
        ...
        assert len(result1.text.strip()) > 0, \
            "First load should extract non-empty text"
        ...
        if load_twice:
            ...
            assert result1.source_path == result2.source_path, \
    ...
    def test_property_1_pdf_structure_preservation(self, pdf_choice):
        ...
        **Validates: Requirements 1.5**
        ...
        assert len(lines) > 1, \
            "Multi-line documents should preserve line structure"
        
        # 2. Text should not be a single continuous string
        # Check for presence of natural breaks
        assert result.text.count('\n') > 0, \
            "Text structure should include line breaks"
        ...
        assert has_structure_indicators, \
            "Document structure indicators should be preserved"
...
class TestTextChunkerProperties:
        ...
        **Validates: Requirements 2.1, 2.2, 2.4**
        ...
        for chunk in chunks:
            ...
            assert chunk.text is not None, \
                "Chunk text should not be None"
            assert chunk.metadata is not None, \
                "Chunk metadata should not be None"
        ...
        for i, chunk in enumerate(chunks):
            assert len(chunk.text) <= max_allowed_size, \
        ...
        for chunk in chunks:
            assert chunk.metadata.source_document == "test_document", \
        ...
        for chunk in chunks:
            ...
            assert chunk.text == expected_text, \
                f"Chunk text should match text[{start}:{end}]"
    
    @given(
        text=st.text(min_size=2000, max_size=5000),
        ...
        if len(text) > chunk_size:
            ...
            for i in range(len(chunks) - 1):
                ...
                if actual_overlap > 0:
                    ...
                    assert overlap_text_from_current == overlap_text_from_next, \
        ...
        **Validates: Requirements 2.1**
        ...
        if len(text) <= chunk_size:
            ...
            assert chunks[0].text == text, \
                "Single chunk should contain all the original text"
            
            # Metadata should reflect full text coverage
            assert chunks[0].metadata.start_position == 0, \
                "Single chunk should start at position 0"
    ...
    def test_property_3_chunk_metadata_completeness(self, text, chunk_size, overlap, source_name):
        ...
        **Validates: Requirements 2.3**
        ...
        for i, chunk in enumerate(chunks):
            assert chunk.metadata is not None, \
                f"Chunk {i} should have metadata"
            ...
            assert chunk.metadata.chunk_index >= 0, \
                f"Chunk {i} chunk_index should be non-negative"
            assert chunk.metadata.chunk_index == i, \
            ...
            assert hasattr(chunk.metadata, 'source_document'), \
        ...
        for i, chunk in enumerate(chunks):
            for field in required_fields:
                assert hasattr(chunk.metadata, field), \
                    f"Chunk {i} metadata missing required field: {field}"
                assert getattr(chunk.metadata, field) is not None, \
    ...
    def test_property_17_chunking_content_preservation(self, text, chunk_size, overlap):
        ...
        **Validates: Requirements 11.1, 11.3, 11.5**
        ...
        assert len(coverage_result.missing_segments) == 0, \
            ...
            f"{coverage_result.missing_segments[:3]}"  # Show first 3 for debugging
        ...
        for i, chunk in enumerate(chunks):
            ...
            expected_text = text[start:end]
            assert chunk.text == expected_text, \
                f"Chunk {i} text should match text[{start}:{end}]"
