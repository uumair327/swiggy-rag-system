...
class TestTextChunker:
    ...
    def test_200_character_overlap(self, chunker):
        ...
        for i in range(len(chunks) - 1):
            ...
            assert actual_overlap >= 150, f"Overlap between chunks {i} and {i+1} is too small: {actual_overlap}"
            assert actual_overlap <= 250, f"Overlap between chunks {i} and {i+1} is too large: {actual_overlap}"
            ...
            assert overlap_text1 == overlap_text2, f"Overlapping text doesn't match between chunks {i} and {i+1}"
    ...
    def test_validate_chunk_coverage_with_overlap(self, chunker):
        ...
        assert len(missing_positions) == 0, f"Missing {len(missing_positions)} positions in coverage"
