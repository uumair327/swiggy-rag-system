...
from core.models import (
    DocumentContent, Chunk, ChunkMetadata, Embedding,
    RetrievedChunk, Answer, IngestionResult, QueryResult,
    CoverageResult
)
...
class TestRAGOrchestrator:
    ...
    def test_ingest_document_complete_workflow_success(
        ...
        embeddings = [
            Embedding(
                ...
                source_text="This is test document"
            ),
            Embedding(
                vector=np.array([0.4, 0.5, 0.6]),
                source_text="document content for ingestion."
            )
        ]
    ...
    def test_ingest_document_without_index_path(
        ...
        chunks = [Chunk(
            text="Test content",
            metadata=ChunkMetadata(0, "/test.pdf", 0, 12)
        )]
        mock_text_chunker.chunk_text.return_value = chunks
    ...
    def test_ingest_document_file_not_found_error(
        self,
        orchestrator,
        mock_document_processor
    ):
    ...
    def test_ingest_document_unexpected_error(
        self,
        orchestrator,
        mock_document_processor
    ):
        ...
        mock_document_processor.load_document.side_effect = RuntimeError(
    ...
    def test_process_query_complete_workflow_success(
        ...
        query_embedding = Embedding(
            vector=np.array([0.1, 0.2, 0.3]),
            source_text="What is the revenue?"
        )
    ...
    def test_process_query_system_not_ready(
        self,
        orchestrator,
        mock_vector_store
    ):
    ...
    def test_process_query_validation_error(
        ...
        # Setup mock to raise ValueError
        mock_query_handler.process_question.side_effect = ValueError(
    ...
    def test_process_query_unexpected_error(
        ...
        # Setup mock to raise unexpected exception
        mock_query_handler.process_question.side_effect = RuntimeError(
    ...
    def test_validate_system_ready_with_embeddings(
        self,
        orchestrator,
        mock_vector_store
    ):
    ...
    def test_validate_system_ready_empty_store(
        self,
        orchestrator,
        mock_vector_store
    ):
    ...
    def test_validate_system_ready_error_handling(
        self,
        orchestrator,
        mock_vector_store
    ):
    ...
    def test_load_index_with_provided_path(
        self,
        orchestrator,
        mock_vector_store
    ):
    ...
    def test_load_index_with_default_path(
        self,
        orchestrator,
        mock_vector_store
    ):
    ...
    def test_load_index_file_not_found(
        ...
        mock_vector_store.load_index.side_effect = FileNotFoundError(
    ...
    def test_load_index_unexpected_error(
        self,
        orchestrator,
        mock_vector_store
    ):
    ...
    def test_error_propagation_from_document_processor(
        self,
        orchestrator,
        mock_document_processor
    ):
