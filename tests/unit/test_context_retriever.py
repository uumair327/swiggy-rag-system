...
class TestContextRetriever:
    ...
    def test_init_with_custom_parameters(self, mock_vector_store):
        ...
        retriever = ContextRetriever(
            vector_store_port=mock_vector_store, top_k=10, similarity_threshold=0.5
        )
    ...
    def test_retrieve_context_top_5_chunks(self, retriever, mock_vector_store, sample_chunks):
        ...
        query_embedding = Embedding(vector=np.array([0.1, 0.2, 0.3]), source_text="test query")
        ...
        mock_vector_store.search.assert_called_once_with(
            query_embedding=query_embedding.vector, top_k=5
        )

    def test_retrieve_context_similarity_threshold_filtering(
        self, retriever, mock_vector_store, sample_chunks
    ):
    ...
    def test_retrieve_context_empty_when_no_chunks_above_threshold(
        self, retriever, mock_vector_store, sample_chunks
    ):
    ...
    def test_retrieve_context_includes_chunk_text_and_metadata(
        self, retriever, mock_vector_store, sample_chunks
    ):
    ...
    def test_retrieve_context_custom_top_k(self, retriever, mock_vector_store, sample_chunks):
        ...
        mock_vector_store.search.assert_called_once_with(
            query_embedding=query_embedding.vector, top_k=2
        )

    def test_retrieve_context_custom_similarity_threshold(
        self, retriever, mock_vector_store, sample_chunks
    ):
        ...
        results = retriever.retrieve_context(query_embedding, similarity_threshold=0.5)
    ...
    def test_retrieve_context_sorted_by_similarity(
        self, retriever, mock_vector_store, sample_chunks
    ):
