...
class DocumentProcessor:
    ...
    def validate_file_path(self, file_path: str) -> ValidationResult:
        ...
        if not os.path.exists(file_path):
            ...
            return ValidationResult(
                is_valid=False,
                error_message=f"Error: PDF file not found at path: {file_path}"
            )
        ...
        if not os.path.isfile(file_path):
            ...
            return ValidationResult(
                is_valid=False,
                error_message=f"Error: Path is not a file: {file_path}"
            )
        ...
        if not file_path.lower().endswith('.pdf'):
            ...
            return ValidationResult(
                is_valid=False,
                error_message=f"Error: File is not a PDF: {file_path}"
            )
        ...
        if not self.document_loader.validate_file(file_path):
            ...
            return ValidationResult(
                is_valid=False,
                error_message=f"Error: File is not readable: {file_path}"
            )
