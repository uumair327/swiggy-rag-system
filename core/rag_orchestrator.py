    ...
    def __init__(
        ...
        if index_path and os.path.exists(index_path + ".faiss"):
            try:
                ...
                logger.info(
                    f"Index loaded successfully with {self.vector_store.get_index_size()} embeddings"
                )
    ...
    def ingest_document(self, file_path: str) -> IngestionResult:
        ...
        try:
            ...
            chunks = self.text_chunker.chunk_text(
                text=document_content.text, source_name=document_content.source_path
            )
            logger.info(f"Created {len(chunks)} chunks")

            # Validate chunk coverage
            coverage_result = self.text_chunker.validate_chunk_coverage(
                original_text=document_content.text, chunks=chunks
            )
            ...
            self.vector_store.add_embeddings(embeddings=embedding_vectors, chunks=chunks)
            ...
            return IngestionResult(
                success=False, chunks_created=0, embeddings_stored=0, error_message=error_msg
            )
    ...
    def process_query(self, question: str) -> QueryResult:
        ...
        try:
            ...
            if not self.validate_system_ready():
                error_msg = (
                    "Error: Vector store not initialized. " "Please ingest a document first."
                )
                logger.error(error_msg)
                answer = Answer(text=error_msg, supporting_chunks=[], confidence="not_found")
                processing_time = time.time() - start_time
                return QueryResult(answer=answer, processing_time_seconds=processing_time)
            ...
            retrieved_chunks = self.context_retriever.retrieve_context(
                query_embedding=query_embedding, top_k=5, similarity_threshold=0.3
            )
            logger.info(f"Retrieved {len(retrieved_chunks)} chunks above similarity threshold")

            if retrieved_chunks:
                avg_similarity = sum(chunk.similarity_score for chunk in retrieved_chunks) / len(
                    retrieved_chunks
                )
            ...
            logger.info(f"Answer generated with confidence: {answer.confidence}")
            ...
            answer = Answer(
                text=f"Error: {error_msg}", supporting_chunks=[], confidence="not_found"
            )
            processing_time = time.time() - start_time
            return QueryResult(answer=answer, processing_time_seconds=processing_time)
