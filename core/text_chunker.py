...
class TextChunker:
    ...
    def chunk_text(
        ...
        while start_pos < len(text):
            ...
            if not self._has_complete_sentence(chunk_text) and end_pos < len(text):
                ...
                if self._has_complete_sentence(extended_text) and len(extended_text) > len(chunk_text):
    ...
    def validate_chunk_coverage(
        ...
        """
        logger.debug(
            f"Validating chunk coverage: {len(original_text)} chars, {len(chunks)} chunks"
        )
        ...
        if not chunks:
            if not original_text:
                ...
                return CoverageResult(
                    is_complete=False,
                    missing_segments=[original_text],
                    duplicate_segments=[]
                )
            ...
            for pos in sorted_missing[1:]:
                if pos == segment_end + 1:
                    ...
                    missing_segments.append(
                        original_text[segment_start:segment_end + 1]
                    )
                    segment_start = pos
                    segment_end = pos
        ...
        if is_complete:
            ...
            logger.warning(
                f"Coverage validation found {len(missing_segments)} missing segments"
            )
